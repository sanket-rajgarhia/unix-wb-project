...
DATE=$(shell date)
TOTAL_NO_OF_LINES=$(shell cat guessinggame.sh | wc -l)
LINES_OF_CODE=$(shell cat guessinggame.sh | grep -vE "(\#|^$$)" | wc -l)
LINES_OF_COMMENT=$(shell cat guessinggame.sh | grep -E '\#' | wc -l)
BLANK_LINES=$(shell cat guessinggame.sh | grep -E '^$$' | wc -l)
